...
import { Edit2, Trash2, Clock, User, MapPin } from 'lucide-react';
import { SectionTimetable, dayNames, timeSlots } from '@/data/timetableData';

interface SectionTimetableTableProps {
  timetables: SectionTimetable[];
  isAdmin?: boolean;
  onEdit?: (timetable: SectionTimetable) => void;
  onDelete?: (id: number) => void;
}

const SectionTimetableTable = ({ 
  timetables, 
  isAdmin = false, 
  onEdit, 
  onDelete 
}: SectionTimetableTableProps) => {
            ...
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="flex items-center space-x-2">
                  <span>{timetable.className} - {timetable.sectionName}</span>
                  <Badge variant="outline">{timetable.academicYear}</Badge>
                </CardTitle>
                <p className="text-sm text-muted-foreground mt-1">
                  Subject-wise Weekly Schedule
                </p>
              </div>
              {isAdmin && (
                      ...
                      >
                        ...
                        <div className="space-y-1">
                          ...
                          <div className="flex items-center space-x-1">
                            ...
                            <span className="text-xs text-muted-foreground">{slot.teacherName}</span>
                          </div>
                          {slot.room && (
                            <div className="flex items-center space-x-1">
...
export default SectionTimetableTable;